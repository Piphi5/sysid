...
void AddMotorController(
  ...
  if (controller == "TalonSRX" || controller == "VictorSPX" ||
    ...
    if (controller == "TalonSRX") {
      ...
      dynamic_cast<WPI_TalonSRX*>(controllers->back().get())
          ->SetStatusFramePeriod(
              motorcontrol::StatusFrameEnhanced::Status_1_General, 5);
      dynamic_cast<WPI_TalonSRX*>(controllers->back().get())
          ->SetStatusFramePeriod(
              motorcontrol::StatusFrameEnhanced::Status_2_Feedback0, 5);
    } else if (controller == "TalonFX") {
      controllers->emplace_back(std::make_unique<WPI_TalonFX>(port));
      dynamic_cast<WPI_TalonFX*>(controllers->back().get())
          ->SetStatusFramePeriod(
              motorcontrol::StatusFrameEnhanced::Status_1_General, 5);
      dynamic_cast<WPI_TalonFX*>(controllers->back().get())
          ->SetStatusFramePeriod(
      ...
      dynamic_cast<WPI_VictorSPX*>(controllers->back().get())
          ->SetStatusFramePeriod(motorcontrol::StatusFrame::Status_1_General_,
                                 5);
      dynamic_cast<WPI_VictorSPX*>(controllers->back().get())
          ->SetStatusFramePeriod(motorcontrol::StatusFrame::Status_2_Feedback0_,
                                 5);
    }
    ...
    static_cast<rev::CANSparkMax*>(controllers->back().get())
        ->SetPeriodicFramePeriod(
            rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus0, 5);
    static_cast<rev::CANSparkMax*>(controllers->back().get())
        ->SetPeriodicFramePeriod(
            rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus1, 5);
    static_cast<rev::CANSparkMax*>(controllers->back().get())
        ->SetInverted(inverted);
  } else {
