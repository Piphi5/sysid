...
static void PrepareGeneralData(const wpi::json& json,
  ...
  auto raw_sf = ComputeAcceleration<4, kVoltageCol, kPosCol, kVelCol>(
      data["slow-forward"], settings.windowSize);
  auto raw_sb = ComputeAcceleration<4, kVoltageCol, kPosCol, kVelCol>(
      data["slow-backward"], settings.windowSize);
  auto raw_ff = ComputeAcceleration<4, kVoltageCol, kPosCol, kVelCol>(
      data["fast-forward"], settings.windowSize);
  auto raw_fb = ComputeAcceleration<4, kVoltageCol, kPosCol, kVelCol>(
  ...
  sysid::TrimStepVoltageData(&raw_ff, settings.stepTestDuration, tempTime);
  sysid::TrimStepVoltageData(&raw_fb, settings.stepTestDuration, tempTime);

  // Trim the step voltage data.
  sysid::TrimStepVoltageData(&ff, settings.stepTestDuration, minStepTime);
  sysid::TrimStepVoltageData(&fb, settings.stepTestDuration, minStepTime);
...
static void PrepareAngularDrivetrainData(
  ...
  // Trim the step voltage data.
  sysid::TrimStepVoltageData(&ff, settings.stepTestDuration, minStepTime);
  sysid::TrimStepVoltageData(&fb, settings.stepTestDuration, maxStepTime);
...
static void PrepareLinearDrivetrainData(
  ...
  auto raw_sfl = ComputeAcceleration<9, kLVoltageCol, kLPosCol, kLVelCol>(
      data["slow-forward"], settings.windowSize);
  auto raw_sbl = ComputeAcceleration<9, kLVoltageCol, kLPosCol, kLVelCol>(
      data["slow-backward"], settings.windowSize);
  auto raw_ffl = ComputeAcceleration<9, kLVoltageCol, kLPosCol, kLVelCol>(
      data["fast-forward"], settings.windowSize);
  auto raw_fbl = ComputeAcceleration<9, kLVoltageCol, kLPosCol, kLVelCol>(
      data["fast-backward"], settings.windowSize);
  auto raw_sfr = ComputeAcceleration<9, kRVoltageCol, kRPosCol, kRVelCol>(
      data["slow-forward"], settings.windowSize);
  auto raw_sbr = ComputeAcceleration<9, kRVoltageCol, kRPosCol, kRVelCol>(
      data["slow-backward"], settings.windowSize);
  auto raw_ffr = ComputeAcceleration<9, kRVoltageCol, kRPosCol, kRVelCol>(
      data["fast-forward"], settings.windowSize);
  auto raw_fbr = ComputeAcceleration<9, kRVoltageCol, kRPosCol, kRVelCol>(
      data["fast-backward"], settings.windowSize);
  ...
  double tempTime =
      0.0;  // Raw data shouldn't be used to calculate mininum step test time
  sysid::TrimStepVoltageData(&raw_ffl, settings.stepTestDuration, tempTime);
  sysid::TrimStepVoltageData(&raw_ffr, settings.stepTestDuration, tempTime);
  sysid::TrimStepVoltageData(&raw_fbl, settings.stepTestDuration, tempTime);
  sysid::TrimStepVoltageData(&raw_fbr, settings.stepTestDuration, tempTime);

  // Trim the step voltage data.
  sysid::TrimStepVoltageData(&ffl, settings.stepTestDuration, minStepTime);
  sysid::TrimStepVoltageData(&ffr, settings.stepTestDuration, minStepTime);
  sysid::TrimStepVoltageData(&fbl, settings.stepTestDuration, minStepTime);
  sysid::TrimStepVoltageData(&fbr, settings.stepTestDuration, minStepTime);

  // Create the distinct raw datasets and store them in our StringMap.
  auto raw_sf = Concatenate(raw_sfl, {&raw_sfr});
  auto raw_sb = Concatenate(raw_sbl, {&raw_sbr});
  auto raw_ff = Concatenate(raw_ffl, {&raw_ffr});
  auto raw_fb = Concatenate(raw_fbl, {&raw_fbr});
  ...
  rawDatasets["Left Forward"] = std::make_tuple(raw_sfl, raw_ffl);
  rawDatasets["Left Backward"] = std::make_tuple(raw_sbl, raw_fbl);
  rawDatasets["Left Combined"] = std::make_tuple(
      Concatenate(raw_sfl, {&raw_sbl}), Concatenate(raw_ffl, {&raw_fbl}));

  rawDatasets["Right Forward"] = std::make_tuple(raw_sfr, raw_ffr);
  rawDatasets["Right Backward"] = std::make_tuple(raw_sbr, raw_fbr);
  rawDatasets["Right Combined"] = std::make_tuple(
      Concatenate(raw_sfr, {&raw_sbr}), Concatenate(raw_ffr, {&raw_fbr}));
